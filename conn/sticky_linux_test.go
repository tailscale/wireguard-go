...
func Test_listenConfig(t *testing.T) {
	t.Run("IPv4", func(t *testing.T) {
		...
		if runtime.GOOS == "linux" {
			...
			sc.Control(func(fd uintptr) {
				i, err = syscall.GetsockoptInt(int(fd), syscall.IPPROTO_IP, syscall.IP_PKTINFO)
			})
			if err != nil {
				t.Fatal(err)
			}
	...
	t.Run("IPv6", func(t *testing.T) {
		...
		if runtime.GOOS == "linux" {
			var i int
			sc.Control(func(fd uintptr) {
				i, err = syscall.GetsockoptInt(int(fd), syscall.IPPROTO_IPV6, syscall.IPV6_RECVPKTINFO)
			})
			if err != nil {
				t.Fatal(err)
			}
