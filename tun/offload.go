...
// GSOSplit splits packets from 'in' into outBufs[n][outOffset:], writing the
// size of each element into sizes. It returns the number of buffers populated,
// and/or an error. Callers may pass an 'in' slice that overlaps with the first
// element of outBuffers, i.e. &in[0] may be equal to &outBufs[0][outOffset].
// GSONone is a valid options.GSOType regardless of the value of
// options.NeedsCsum. Length of each outBufs element must be greater than or
// equal to the length of 'in'.
