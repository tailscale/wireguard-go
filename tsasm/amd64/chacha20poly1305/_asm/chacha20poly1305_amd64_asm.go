...
// outputPath returns the path Generate() was told to write, so the
// post-processing below edits the file we just generated. Upstream hardcoded
// "../chacha20poly1305_amd64.s", which is correct only when generating in
// place; the reproducibility test generates into a temporary directory, and
// with the hardcoded path it would post-process the checked-in file instead.
