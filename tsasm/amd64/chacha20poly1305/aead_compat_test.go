...
// This file follows tsasm/arm/chacha20poly1305/aead_compat_test.go, which was
// in turn derived from device/aead_compat_test.go (originally MIT-licensed by
// WireGuard LLC). The aeadCtors slice names our asm-backed implementation
// alongside the reference pure-Go path so every test and benchmark runs
// against both.
...
// TestAEAD_RFC8439Vector verifies each implementation against the vector from
// RFC 8439 section 2.8.2, for both Seal and Open. A round-trip-only test (one
// implementation decrypting its own output) would pass even if the whole
// construction were symmetrically wrong; this will not.
...
// sealSizes covers every length boundary the SSE kernel branches on. The
// kernel has a dedicated path for buffers up to 128 bytes (openSSE128), a
// 4x64-byte main loop, dedicated 192- and 320-byte entries, and a tail
// handler that works in 64-, 16- and 1-byte steps, so each of those
// boundaries is probed at -1, exact and +1.
var sealSizes = []int{
...
// TestAEAD_Concurrent exercises Seal and Open from many goroutines to catch
// shared-state bugs. The kernel keeps all state on the stack, so this should
// be uneventful; it is here because a regression would be catastrophic and
// silent.
